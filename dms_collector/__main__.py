...
foopts.add_argument('--csvdelimiter', required=False, default=",", metavar='<char>', help='CSV delimiter')    
...
otopts.add_argument("-V", '--version', action='version', version='%(prog)s ' + __version__ + ', supports DMS tbml versions: ' + ','.join(TBML_VERSIONS))
otopts.add_argument('--noversioncheck', required=False, help='do not check tbml version',default=False,action='store_true')
otopts.add_argument('--readtimeout', required=False, help='htto read timeout, default is %s seconds'%30,type=float,default=30,metavar='<secs>')   
otopts.add_argument('--basicauth', required=False, default=False,action='store_true',help='use basic authentication instead of form login. This is required for earlier versions of DMS SPy.') 
